...
for i=1:10000
    ...
    if mod(i,100) == 0
        @save "data/US/model_parameters.jld"  β γ δ J
        I=u[:,2]
        R=u[:,3]
        D=u[:,4]
        global P=scatter(uₓ[OutbreakDate:Day(1):LastDate][:Confirmed],legend=:topleft,color=:orange,title="US")
...
#        pP=plot(DataWindow.*βₘ,lw=0,color=:whitesmoke,fill=(0,:whitesmoke),α=0.9,legend=:topleft)
        ...
        debuggP=plot(β[AssimTimeRange[1]:AssimTimeRange[1]+12],label=L"\beta",legend=:left,
                     lw=3,title="US",color=:red,tickfontsize=12)
        savefig(debuggP,"figs/US/dbug.pdf")
...
writetimearray(Parameter, "data/US/Parameter.csv")#Save data
################### Prognosis ################
u=corona.forward(β,γ,δ,u₀,SimulTimeSpan)
Prognosis=TimeArray(SimulTime,u,["S","I","R","D"])
@save "data/US/final.jld" Prognosis uₓ β γ δ u₀ SimulTime SimulTimeSpan SimulTimeRange
writetimearray(Prognosis, "data/US/Prognosis.csv")
cd(startdir)
