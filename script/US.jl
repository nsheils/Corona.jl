...
S₀=Float64(Population_0[Population_0[!,:Column1].=="US",:][!,:Column2][1])
...
Data=merge(
    TimeSeries.rename(Confirmed[Symbol("US")],:Confirmed),
    TimeSeries.rename(Deaths[Symbol("US")],:Deaths ))
...
OutbreakDate=Outbreak[Symbol("US")]
...
u₀=[S₀,Float64(values(Confirmed[Symbol("US")][OutbreakDate])[1]),0.0,0.0]
β₀=1/4*ones(nSimul+1)
γ₀=1/7*ones(nSimul+1)
...
########################################
β,γ,δ=β₀,γ₀,δ₀
@load "data/US_model_parameters.jld"
...
for i=1:20000
    ...
    u=corona.forward(β,γ,δ,u₀,AssimTimeSpan)
    if mod(i,1000) == 0
        @save "data/US_model_parameters.jld" FirstDay β γ δ J
        I=u[:,2]
        R=u[:,3]
        D=u[:,4]
        global J=[J; norm([Cₓ;Dₓ]-[u[:,2]+u[:,3]+u[:,4];u[:,4]])/norm([Cₓ;Dₓ])]
        println(i," ",J[end])
        global P=scatter(uₓ[AssimTime][:Confirmed],legend=:topleft,color=:orange,title="US")
        ...
        global lP=scatter(uₓ[AssimTime][:Confirmed].+1,legend=:topleft,
            color=:orange,title="US",yaxis=:log10,tickfontsize=12)
...
#        savefig(lP,"figs/US_CIR_log.pdf")
        ...
        display(pP)
        savefig(pP,"figs/US_parameters3.pdf")
   end
end
## prolong parameters
...
################### Prognosis ################
u=corona.forward(β,γ,δ,u₀,SimulTimeSpan)
Prognosis=TimeArray(SimulTime,[u β[SimulTimeRange] γ[SimulTimeRange] δ[SimulTimeRange]],["S","I","R","D", "β",  "γ",  "δ" ])
@save "data/US_final.jld" Prognosis Data
PrognosisPlot=scatter(uₓ[DataTime],label=["Confirmed" "Deaths"],
            title="Prognosis US",legend=:left,color=[:orange :black])
PrognosisPlot=plot!(SimulTime, u,label=["S" "I" "R" "D"],lw=3,color=[:blue :red :green :black])

savefig(PrognosisPlot,"figs/US_Prognosis.pdf")
...
WeekPrognosisPlot=scatter(uₓ[DataTime],label=["Confirmed","Deaths"],
            title="One Week Prognosis US",legend=:left,color=[:orange :black])
...
savefig(WeekPrognosisPlot,"figs/US_Prognosis.pdf")
...
savefig(MonthPrognosisPlot,"figs/US_MonthPrognosis.pdf")


writetimearray(Prognosis[MonthTime], "data/US_MonthPrognosis.csv")
writetimearray(Parameter[MonthTime], "data/US_MonthParameter.csv")
writetimearray(Prognosis[WeekTime], "data/US_WeekPrognosis.csv")
writetimearray(Parameter[WeekTime], "data/US_WeekParameter.csv")
