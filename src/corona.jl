...
using TimeSeries
# %%
cd("/home/jls/prog/corona/figs/")
data="/home/jls/data/COVID-19/csse_covid_19_data/csse_covid_19_time_series/time_series_19-covid-Confirmed.csv";
Confirmed=CSV.read(data);
FirstDate=Date(String(names(Confirmed)[5]),"m/d/y")+Dates.Year(2000)
LastDate=Date(String(names(Confirmed)[end]),"m/d/y")+Dates.Year(2000)
DateRange=FirstDate:Day(1):LastDate
UK_confirmed=convert(Matrix,filter(g -> g[2] =="United Kingdom", Confirmed)[5:end])
UK_confirmed=reshape(sum(UK_confirmed,dims=1),:,1)
Germany_confirmed=reshape(convert(Matrix,filter(g -> g[2] =="Germany", Confirmed)[5:end]),:,1)
...
pv=plot(trend(Cases[:I],24),label="I",lw=3,yaxis=(0,5),legend=:topleft,ylabel="Verdopplung")
pv=plot!(trend(Cases[:D],18),label="D",lw=3,yaxis=(0,5))
pv=plot!(trend(Cases[:F],18),label="F",lw=3,yaxis=(0,5))
pv=plot!(trend(Cases[:E],19),label="E",lw=3,yaxis=(0,5))
pv=plot!(trend(Cases[:US],13),label="US",lw=3,yaxis=(0,5))
pv=plot!(trend(Cases[:Ch],28),label="CH",lw=3,yaxis=(0,5))
