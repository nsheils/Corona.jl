...
import Plots.plot
function plot(da::DA,region::String,yaxis=:none::Symbol)
    if yaxis == :log10
        add = 1.0
    else
        add = 0.0
    ...
    scatter(da.data.Confirmed[datatime],color=:orange,
                Atickfontsize=12,title=region,legend=:topleft,yaxis=yaxis)
    scatter!(da.data.Deaths[datatime].+add,color=:black)
    plot!(da.u.I .+ da.u.R .+ da.u.D,legend=:topleft,color=:orange,lw=3,label="C")
